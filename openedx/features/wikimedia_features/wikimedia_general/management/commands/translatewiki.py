...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '-u', '--username', help='git username', default='edx-platfrom',
        )
        parser.add_argument(
            '-e', '--email', help='git email', default='edx@edx.com',
        )
    ...
    def _clone_repo_or_pull_latest_changes(self, token, username, repo_name, to_directory='.'):
        """
        Clone or pull latest changes from master branch
        Arguments:
            token: (str) Github access token
            username: (str) Github username
            repo_name: (str) Github repository name
            to_directory: (str) Location to clone github repo
        """
        repo_location = '{}/{}'.format(to_directory, repo_name)
        if not os.path.exists(repo_location):
            log.info(f'cloning {repo_name} to {repo_location}')
            origin = f'https://{token}@github.com/{username}/{repo_name}.git'
            execute(f'git clone {origin}', '.')
        else:
            log.info(f'Updating {repo_location} master branch')
            execute(f'git pull origin master', repo_location)
    
    def _push_chnages_to_repository(self, repo_name, git_username, git_email, to_directory='.'):
        """
        Push the changest to git repository
        Arguments:
            repo_name: (str) Github repository name
            git_username: (str) Username on commit message
            git_email: (str) Email on commit message
            to_directory: (str) repository path
        """
        repo_location = '{}/{}'.format(to_directory, repo_name)
        if os.path.exists(repo_location):
            log.info('Trying commiting latest changes to github')
            execute(f'git config user.name "{git_username}"', repo_location)
            execute(f'git config user.email "{git_email}"', repo_location)
            execute("git add .;git commit -m 'Translations Update';git push origin master", repo_location)
        else:
            raise ValueError(f"Translatewiki: Can't push files becuase repo doesn't exist: {repo_location}")

    def _copy_translation_files_from_src_to_dest(self, src_dir, dest_dir, locales, files):
        """
        Copy files from source translation directory to destination directory
        Arguments:
            src_dir: (str) source root directory i.e 'conf/locale'
            dest_dir: (str) destination root directory i.e 'edx-platform-translations'
            locales: (list) list of languages i.e ['ar', 'en', 'fr']
            files: (list) files to copy i.e ['wiki.po', 'wikijs.po']
        """
        for lang in locales:
            src_path = f'{src_dir}/{lang}/LC_MESSAGES'
            dest_path = f'{dest_dir}/{lang}/LC_MESSAGES'
            self._validating_files(src_path, files)
            log.info(f'Copying files from {src_path} to {dest_path}')
            for file_name in files:
                os.makedirs(os.path.dirname(f'{dest_path}/{file_name}'), exist_ok=True)
                shutil.copy(f'{src_path}/{file_name}', f'{dest_path}/{file_name}')
    ...
    def pull(self, locales, staged_files, update_edx_files=True):
        """
        Clone or Update the repository and copy files from repo to edx directory
        Arguments:
            locales: (list) list of languages i.e ['ar', 'en', 'fr']
            staged_files: (list) files to copy i.e ['wiki.po', 'wikijs.po']
            update_edx_files: (bool) if Fasle it will not update edx translation files
        """
        git_access_token = os.getenv('TRANSLATION_REPO_ACCESS_TOKEN')
        git_organization = os.getenv('TRANSLATION_REPO_ORGANIZATION')
        git_repo_name = os.getenv('TRANSLATION_REPO_NAME')
        if not git_access_token:
            raise ValueError("token not found, please set TRANSLATION_REPO_ACCESS_TOKEN as an env variable")
        if not git_organization:
            raise ValueError('organization not found, please set TRANSLATION_REPO_ORGANIZATION as an env variable')
        if not git_repo_name:
            raise ValueError('repository-name not found, please set TRANSLATION_REPO_NAME as an env variable')

        self._clone_repo_or_pull_latest_changes(
            git_access_token, git_organization, git_repo_name,
        )
        if update_edx_files:
            repo_files = set(os.listdir(git_repo_name))
            locales = list(set(locales).intersection(repo_files))
            self._copy_translation_files_from_src_to_dest(
                git_repo_name, self.EDX_TRANSLATION_PATH, locales, staged_files,
            )
    
    def push(self, locales, staged_files, git_username, git_email, updete_remote_repo=True):
        """
        Copy files from edx to repository and Push changes to master branch of 
        the translation repository
        Arguments:
            locales: (list) list of languages i.e ['ar', 'en', 'fr']
            staged_files: (list) files to copy i.e ['wiki.po', 'wikijs.po']
            git_username: (str) git username on commit message
            git_email: (str) git email on commit message
            updete_remote_repo: (bool) if Fasle it will not update remote repository
        """
        git_repo_name = os.getenv('TRANSLATION_REPO_NAME')
        if not git_repo_name:
            raise ValueError('repository-name not found, please set TRANSLATION_REPO_NAME as an env variable')
        ...
        self._copy_translation_files_from_src_to_dest(
            self.EDX_TRANSLATION_PATH, git_repo_name, locales, staged_files
        )
        if updete_remote_repo:
            self._push_chnages_to_repository(git_repo_name, git_username, git_email)
    ...
    def handle(self, *args, **options):
        ...
        wm_files = [f'wm-{filename}' for filename in targated_files]
        ...
        if options['action'] == 'pull':
            self.pull(locales, wm_files)
        elif options['action'] == 'push':
            username = options.get('username')
            email = options.get('email')
            self.push(locales, wm_files, username, email)
